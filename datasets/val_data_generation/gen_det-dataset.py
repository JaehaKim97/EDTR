...
def main(args) -> None:
    ...
    # Making dataset:
    for gt_list, lq_list, annot_list, path_list in tqdm(loader):
        gt_list = list(rearrange(gt, 'h w c -> c h w').contiguous().float() for gt in gt_list)
        lq_list = list(rearrange(lq, 'h w c -> c h w').contiguous().float() for lq in lq_list)
        annot_list = [{k: v.long() if isinstance(v, torch.Tensor) else v for k, v in t.items()} for t in annot_list]
