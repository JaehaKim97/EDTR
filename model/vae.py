...
class SDPAttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = rearrange(out, 'b (h w) c -> b c h w', b=B, h=H, w=W, c=C).contiguous()  # Here to solve the problem?
        out = self.proj_out(out)
        return x+out
...
class Unraveler(nn.Module):
    ...
    def forward(self, x0, output):
        # timestep embedding
        temb = None
        x = torch.cat((x0, output), dim=1)
...
class AutoencoderKL(nn.Module):
    ...
    def __init__(self, ddconfig, embed_dim,
        ...
        frozen_modules = [self.quant_conv, self.post_quant_conv]
        if not self.train_encoder:
            frozen_modules.append(self.encoder)
        if not self.train_decoder:
            frozen_modules.append(self.decoder)
