...
def main(args) -> None:
    ...
    for gt, lq, _, gt_path in tqdm(loader):
        ...
        for idx, img_name in enumerate(gt_path):
            # save gt
            img_name = os.path.splitext(os.path.join(exp_dir, 'gt', *gt_path[idx].split('/')[-1:]))[0] + ".png"
            os.makedirs(os.path.dirname(img_name), exist_ok=True)
            save_image(gt[idx], img_name)
            
            # save lq
            img_name = os.path.splitext(os.path.join(exp_dir, 'lq', *gt_path[idx].split('/')[-1:]))[0] + ".png"
            os.makedirs(os.path.dirname(img_name), exist_ok=True)
