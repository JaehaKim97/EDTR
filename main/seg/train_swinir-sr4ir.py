...
from utils.common import (
    ...
    set_logger, print_attn_type, calculate_psnr_pt
)
...
def main(args) -> None:
    ...
    swinir, segnet, opt_swinir, opt_segnet, sch_swinir, sch_segnet, loader, val_loader = \
        accelerator.prepare(swinir, segnet, opt_swinir, opt_segnet, sch_swinir, sch_segnet, loader, val_loader)
    ...
    epoch = 0
    loss_records = dict(swinir_pix=[], swinir_tdp=[], segnet=[])
    ...
    while global_step < max_steps:
        ...
        for gt, lq, mask, _ in loader:
            ...
            with accelerator.autocast():
                ...
                _, feat_gt = segnet(gt, normalize=True, return_feat=True)
                _, feat_res = segnet(res, normalize=True, return_feat=True)
                loss_swinir_tdp = F.l1_loss(input=feat_res['C5'], target=feat_gt['C5'], reduction="mean")
            ...
            with accelerator.autocast():
                ...
                cqmix = res*cqmix_mask + gt*(1-cqmix_mask) 
                img_cat = torch.cat((res, gt, cqmix), dim=0)
                mask_cat = torch.cat((mask, mask, mask), dim=0)
                pred = segnet(img_cat, normalize=True)
                loss_segnet = F.cross_entropy(pred["out"], mask_cat, ignore_index=255)
            opt_segnet.zero_grad()
            accelerator.backward(loss_segnet)
            ...
            loss_records["segnet"].append(loss_segnet.item())
            pbar.update(1)
